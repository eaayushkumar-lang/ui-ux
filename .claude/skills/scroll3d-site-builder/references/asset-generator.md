...
## Scope
Produces ONE ready-to-use asset (a video file, or a still image) for ONE section, based
on that section's `content_brief` and `motion` from the site plan. This does not build
sites, decide structure, or make creative direction calls beyond translating a
content_brief into a generation prompt. Consult this file whenever `video-scroll-effect.md`,
`3d-scene-effect.md`, or `pointer-follow-effect.md` say a section needs a generated/ingested asset.

## Step 1 — Check for a user-supplied asset first
Before generating anything, check for a source asset in this order:
1. **`section.user_asset`** — if the plan carries one (supplied by the user during the
   planning review), use it explicitly. This is the preferred, explicit source — the
   plan already told you this is what the user wants here.
...
- If the file isn't valid for what's needed, say so plainly and ask whether to fall
  back to generation.
...
- Motion prompt from the section's `motion` value (see `video-scroll-effect.md`'s motion
  patterns for phrasing examples — orbit, fly-through, reveal/explode, abstract, plus
  the richer named set there).
- Preflight cost with `get_cost:true`; confirm with the user if cumulative cost
  across sections is non-trivial (this confirmation is coordinated once across the
  whole plan during the planning review in `SKILL.md`, not repeated per section here).
...
## Step 4 — Poll, download, extract
Once a generation job is submitted, poll it (via `job_display` or equivalent) rather
than blocking the main flow of conversation on a long render wait, download the result
once complete, and — for video — extract frames per `video-scroll-effect.md`'s Step 5.
For a multi-beat section, do this once per beat (each beat's clip extracts into its own
`frames/<section>/<beat>/` folder); the caller still gets one combined result covering
all beats.
...
"actual_frame_count": 179, // only if extracted
...
different, non-deviating case. This field is what lets `video-scroll-effect.md`'s
"STOP — do not self-reroute" step and `SKILL.md`'s "Disclosing plan deviations" rule
actually work — a deviation can't be disclosed if it was never surfaced here.

## What this never does
- Never decides site structure, section count, or content — receives a
  `content_brief`, doesn't originate one.
- Never builds the actual HTML/CSS/site output — that's the executing technique's
  job, using what this produces.
- Never retries a failed generation more than once with the same prompt — reword
  or switch models (per `video-scroll-effect.md`'s retry guidance) rather than
