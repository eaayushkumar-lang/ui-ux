/**
 * Rotating earth visual. Adapted from the supplied component with one
 * necessary fix: the outer wrapper used `h-screen` (forces full viewport
 * height), which only makes sense as a full-page demo - embedded inside
 * the Hero's existing aspect-square visual column it would blow the
 * layout out to 100vh. Swapped to `h-full` so it fills whatever container
 * it's placed in instead. Everything else (globe size, star positions,
 * animation timings) is unchanged from the source.
 */
