...
export function Faq() {
  ...
  return (
    <section id="faq" className="px-5 py-20 sm:px-8 sm:py-28 md:px-12 md:py-32">
          ...
          {FAQS.map((item, i) => {
            ...
            return (
              <Reveal
                key={item.q}
                delay={180 + i * 80}
                className="overflow-hidden rounded-2xl border border-white/15 bg-white/5 backdrop-blur-md"
              >
                <button
                  type="button"
                  onClick={() => setOpen(isOpen ? -1 : i)}
                  aria-expanded={isOpen}
                  className="flex w-full items-center justify-between gap-4 px-5 py-5 text-left text-base font-medium text-white transition-colors duration-300 hover:bg-white/5 sm:px-6"
                >
                  {item.q}
